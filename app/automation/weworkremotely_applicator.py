"""
WeWorkRemotely Applicator Module
"""
...
class WeWorkRemotelyApplicator(BaseApplicator):
    ...
    async def login_if_needed(self) -> bool:
        """No login needed for WeWorkRemotely."""
        return True
    ...
    async def apply(self, job_data: Dict, resume_data: Dict) -> ApplicationResult:
        ...
        try:
            ...
            apply_button = await self.page.query_selector('a:has-text("Apply for this position")')
            if not apply_button:
                return self.create_result(
                    job_data,
                    'skipped',
            ...
            logger.error(f"Error applying on WeWorkRemotely: {str(e)}")
            return self.create_result(
                job_data,
                'failed',
                str(e)
    ...
    async def _get_external_link(self) -> Optional[str]:
        ...
        try:
            ...
            how_to_apply = await self.page.query_selector('.how-to-apply')
            if how_to_apply:
                link = await how_to_apply.query_selector('a')
                if link:
                    return await link.get_attribute('href')
            
            # Check for direct application link
    ...
    async def _extract_email_from_description(self) -> Optional[str]:
        ...
        try:
            # Check listing body
            description = await self.page.text_content('.listing-container')
            if description:
                email_match = re.search(r'[\w\.-]+@[\w\.-]+\.\w+', description)
                if email_match:
                    return email_match.group(0)
            
            # Check how to apply section
            how_to_apply = await self.page.text_content('.how-to-apply')
            if how_to_apply:
                email_match = re.search(r'[\w\.-]+@[\w\.-]+\.\w+', how_to_apply)
                if email_match:
                    return email_match.group(0)
